...
function blockHover(status)
{
	$(document).off('mouseenter').on('mouseenter', '.product_list.grid li.ajax_block_product .product-container , ul.tm-carousel li.item .product-container' , function(e){
		if ('ontouchstart' in document.documentElement)
			return;
		if ($('body').find('.container').width() == 1070)
		{
...
function accordion(status)
{
	...
	if(status == 'enable')
	{
		$('#right_column .block .main_title, #left_column .block .main_title, #left_column #newsletter_block_left h4').on('click', function(){
	...
	{
		$('#right_column .block .main_title, #left_column .block .main_title, #left_column #newsletter_block_left h4').removeClass('active').off().parent().find('.block_content').removeAttr('style').slideDown('fast');
